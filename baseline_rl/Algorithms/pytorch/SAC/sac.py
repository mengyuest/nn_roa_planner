...
class SAC:
    ...
    def _init_model(self):
        ...
        if self.args.pretrained_path is not None:
            print("Loading my model %s"%(self.args.pretrained_path))
            self.policy_net, self.value_net, self.q_net_1, self.q_net_2, self.running_state \
                = pickle.load(open(self.args.pretrained_path, "rb"))
    ...
    def learn(self, writer, i_iter, offset=0):
        """interact"""
        ...
        print(f"Iter: {i_iter+offset}, num steps: {log['num_steps']}, total reward: {log['total_reward']: .4f}, "
