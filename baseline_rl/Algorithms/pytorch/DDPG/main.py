...
def define_parser():
    ...
    # @click.command()
    parser.add_argument("--env_id", type=str, default="HalfCheetah-v3", help="Environment Id")
...
def main(rest_args=None):
    ...
    for i_iter in range(1, args.max_iter + 1):
        ...
        if i_iter==1 or i_iter % 10 == 0:
            if hasattr(ddpg.env, "print_stat"):
                ddpg.env.print_stat()

        if i_iter % args.eval_iter == 0:
            ddpg.eval(i_iter, render=args.render)

        if i_iter % args.save_iter == 0:
            ddpg.save(model_path)
            global_patch.save(ddpg, model_path, i_iter)  # TODO(yue)
